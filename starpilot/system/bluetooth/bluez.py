...
class BlueZClient:
  ...
  def pair(self, address: str) -> None:
    ...
    try:
      self._call(device["path"], DEVICE_IFACE, "Pair", timeout=30.0)
    except (RuntimeError, TimeoutError) as error:
      # If device is a BLE peripheral (or rejects SSP/PIN pairing handshake),
      # fallback to Trust + Connect which establishes the BLE link cleanly.
      error_msg = str(error).lower()
      if not error_msg or any(term in error_msg for term in ("timeout", "not ready", "already", "authenticationfailed", "authentication rejected", "failed")):
        self.set_device_property(address, "Trusted", "b", True)
        self.connect(address)
      else:
        raise
